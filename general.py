...
def check_argv(argv):
    ...
    # check number of input arguments is correct# change from 3 , because extra gui_param
    if len(argv) != 4:
        raise IOError(usage + "\n\tIncorrect number of input arguments\n")

    # check parameters file is a .txt or .xml file
    #elif (".xml" not in argv[0]) and (".txt" not in argv[0]):
        #raise IOError(usage + "\n\t" + argv[0] + " is not a .txt or .xml file\n")
    ...
    elif (".inp" not in argv[2]) and (".ntr" not in argv[2]) and (
            ".neutral" not in argv[2] and (".cdb" not in argv[2])):
...
def check_argv_second(argv):
    ...
    # check Nodes is a list file
    elif ".lis" not in argv[2]:
        raise IOError(usage + "\n\t" + argv[2] + " is not Node list file\n")

    # check Elements is a list file
    elif ".lis" not in argv[3]:
        raise IOError(usage + "\n\t" + argv[3] + " is not Node list file\n")

    # check Elements & Nodes file exists
    elif (os.path.isfile(argv[2])) and (os.path.isfile(argv[3])) == False:
        raise IOError(usage + "\n\t" + argv[2] + " & " + argv[3] + " does not exist\n")
